...
u8 NAND_Init(void)
{
	...
	GPIO_Set(GPIOD,(3<<0)|(3<<4)|(3<<11)|(3<<14),GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);//PD0,1,4,5,11,12,14,15 AF OUT
	GPIO_Set(GPIOE,(0XF<<7),GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);	//PE7~10,AF OUT
	GPIO_Set(GPIOG,(1<<9),GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);		//PG9 AF OUT
	...
	FMC_Bank3->PCR=tempreg;		//设置NAND控制寄存器
	...
	FMC_Bank3->PMEM=tempreg;	//设置通用时序寄存器 
	FMC_Bank3->PATT=0;			//设置特性时序寄存器 
	  
	FMC_Bank3->PCR|=1<<2;		//使能存储区3 
