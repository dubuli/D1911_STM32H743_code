...
{  
	...
	DMA1_Stream1->PAR=(u32)&DCMI->DR;//外设地址为:DCMI->DR
	...
	if(mem1addr)					//双缓冲的时候,才需要开启
	{
		...
		MY_NVIC_Init(2,3,DMA1_Stream1_IRQn,2);//抢占1，子优先级3，组2  
	} 
...
void DCMI_Init(void)
{
	...
	GPIO_Set(GPIOA,PIN6,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);			//PA6复用功能输出
	GPIO_Set(GPIOB,PIN7|PIN8|PIN9,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);	//PB7/8/9复用功能输出
	GPIO_Set(GPIOC,PIN6|PIN7|PIN8|PIN9|PIN11,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);//PC6/7/8/9/11 复用功能输出
	GPIO_Set(GPIOD,PIN3,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);			//PD3复用功能输出
	GPIO_Set(GPIOH,PIN8,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);			//PH8复用功能输出 
