...
u8 FTL_ReadSectors(u8 *pBuffer,u32 SectorNo,u16 SectorSize,u32 SectorCount)
{
    ...
    for(i=0;i<SectorCount;i++)
    {
        ...
        //出现TWOBIT错误时，对读取数据特殊处理，增强稳定性
        if (flag==NSTA_ECC2BITERR) 
		{
            
            NAND_ReadSpare(PBNNo*nand_dev.block_pagenum,0,buf_temp,4);					//读取4个字节,以判断该块是否是使用 
            for (j = 0; j < 1; j++) 
			{                                
				if ((buf_temp[1] != 0xFF) ) 											//表明是使用的块，two bit error处理
				{										
					flag=NAND_ReadPage(PhyPageNo,PageOffset,pBuffer,rsecs*SectorSize);			//重读数据,再次确认
					if(flag==NSTA_ECC1BITERR)
					{
						...
						flag=0;
					}else if (flag== NSTA_ECC2BITERR) printf("2bit ecc error or more\r\n"); 
				}else flag=0;				//2bit ecc错误,该块是未使用的块， 不处理(可能是初次写入数据导致的) 
            }
        }
		if(flag)return 2;					//失败
