...
//V1.1 20160822
//1,修改为使用硬件JPEG解码视频
//2,新增横竖屏自动切换,以支持高分辨率的视频
//V1.2 20180830
//1,针对NAND,修改video_play_mjpeg函数,增加MPU保护设置相关代码,防止NAND访问出错
////////////////////////////////////////////////////////////////////////////////// 	   
...
{				 	 
	...
	while(rval==0)//主循环
	{
		...
		{	
			...
			videodev.mfindextbl=(u32*)gui_memin_malloc(flistbox->filecnt*4);//为新的tbl申请内存
...
extern vu8 memshow_flag;//mem打印使能标志
//播放mjpeg文件
//返回值:
//0,正常播放结束,返回文件浏览状态.
...
{    
	u8 dirmode=0;		//横竖屏模式:0,竖屏模式;1,横屏模式.
	u8 *pname=0;		//带路径的名字
	...
	if(videodev.path[0]=='2')	//如果图片是存放在NAND的,则MPU禁止CACHE和缓冲,同时关闭D Cache,以确保访问正常
	{
		MPU_Set_Protection(0XC0000000,32*1024*1024,2,MPU_REGION_FULL_ACCESS,0,0,0);	//保护SDRAM区域,共32M字节,禁止共用,禁止cache,禁止缓冲
 	}
	...
	{ 
		...
		if(res==0)
		{
			...
			{
				if(avix.Height>=480 && avix.Width>=800 && (1000000/avix.SecPerFrame) >=15)	//视频分辨率大于等于800*480,且帧率大于等于15帧的时候,需要横屏显示,已满足速度要求.
				{ 
					dirmode=1;			//横屏模式
					LCD_Display_Dir(1);	//改为横屏	
					yoff=0;
			...
			TIM6_Int_Init(avix.SecPerFrame/100-1,20000-1);	//10Khz计数频率,加1是100us,用于视频帧间隔控制
			...
			res=mjpeg_init((lcddev.width-avix.Width)/2,yoff+(videoheight-avix.Height)/2,avix.Width,avix.Height);//JPG解码初始化 
					...
					{ 
						...
						{ 
							...
							{ 
								...
								//printf("shit1:%d\r\n",avix.StreamSize); 
								SAI_Play_Stop();	//关闭音频
								if(video_seek_vids(&videodev,&avix,framebuf))//查找下一个视频帧
								{
									playflag=0XFE;	//播放错误，跳到下一个视频
									break;			//电影结束了
								} 
...
//avi 视频标志查找,用于出错后查找视频帧
//videodevx:视频控制器
//aviinfo:avi文件信息
//mbuf:视频缓冲帧地址
//返回值:0,成功;其他,失败.
u8 video_seek_vids(__videodev *videodevx,AVI_INFO *aviinfo,u8 *mbuf)
{
	u32 fpos;
	u8 *pbuf;
	u32 br;  
	...
	{	
		...
		pbuf=mbuf;
		br=avi_srarch_id(pbuf,AVI_VIDEO_BUF_SIZE,avix.VideoFLAG);	//寻找视频帧
		if(br!=0XFFFFFFFF)
		{
			avi_get_streaminfo(pbuf+br);		//获取流信息 
			if(avix.StreamSize<100||avix.StreamSize>AVI_VIDEO_BUF_SIZE)continue;	//错误的视频帧尺寸
