...
extern vu8 system_lwip_sta;//lwip任务运行标志
//LWIP初始化(LWIP启动的时候使用)
//返回值:0,成功
//      1,内存错误
//      2,LAN8720初始化失败
...
void lwip_comm_destroy(void)
{
...
#endif
	HAL_ETH_Stop_IT(&LAN8720_ETHHandle);//停止ETH中断传输

	RCC->AHB1RSTR|=1<<15;			//复位ETH MAC
	delay_ms(100);
	...
	memset(&LAN8720_ETHHandle,0,sizeof(ETH_HandleTypeDef));	//LAN8720_ETHHandle结构体数据清零
