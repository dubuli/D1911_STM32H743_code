...
//ALIENTEK 阿波罗STM32F767开发板 实验65
//综合测试 实验 

//注意:综合测试实验,需要修改sys.c里面的PLL2倍频系数为432,以得到108M的SDRAM频率
//     综合实验最多跑108M的SDRAM时钟,否则部分应用(如:FC游戏机)可能异常!!!
...
//显示版权信息
	...
	LCD_ShowString(icowidth+5*2,ypos+fsize*j++,lcddev.width,lcddev.height,fsize, "ALIENTEK STM32F4/F7/H7");
...
//开始硬件检测初始化
	...
	LCD_ShowString(5,ypos+fsize*j++,lcddev.width,lcddev.height,fsize, "CPU:STM32H743IIT6 400Mhz");
	LCD_ShowString(5,ypos+fsize*j++,lcddev.width,lcddev.height,fsize, "FLASH:2048KB  SRAM:1060KB");	 
	...
	while(W25QXX_ReadID()!=W25Q256)
	{
		...
		if(res>10)break;
		delay_ms(50);
		W25QXX_Init();	//重新初始化W25Q256
	}
	if(res>=10)//10次都检测不到
	{	 
		system_error_show(5,ypos+fsize*j++,"QSPI Flash Error!!",fsize); 
	}else temp=32*1024;			//32M字节大小
	LCD_ShowString(5,ypos+fsize*j,lcddev.width,lcddev.height,fsize, "QSPI Flash:     KB");			   
	LCD_ShowxNum(5+11*(fsize/2),ypos+fsize*j,temp,5,fsize,0);//显示spi flash大小  
	LCD_ShowString(5+okoffset,ypos+fsize*j++,lcddev.width,lcddev.height,fsize, "OK");   
	//检测是否需要擦除SPI FLASH?
	res=KEY_Scan(1);//
	if(res==KEY2_PRES)
	...
	if(res==0X0D)//文件系统不存在
	{
		...
		res=f_mkfs("1:",FM_ANY,0,fatbuf,FF_MAX_SS);		//格式化SPI FLASH,1:,盘符;FM_ANY,自动选择文件系统类型,0,自动选择簇大小
		if(res==0)
		{
	...
	if(font_init()||(res==1))//检测字体,如果字体不存在/强制更新,则更新字库	
	{
		...
		{
			TIM8_Int_Init(100-1,20000-1);//启动TIM8 轮询USB,10ms中断一次	
...
vu8 system_lwip_sta=0;		//是否有lwip任务正在运行.
//监视任务
void watch_task(void *pdata)
{
    OS_CPU_SR cpu_sr=0; 
